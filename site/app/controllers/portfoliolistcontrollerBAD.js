portfolioApp.controller('PortfolioListController', function( $scope, $firebaseArray ) {
	...
	$scope.favorites = [];
	// To get animation to work, keep track of picked portfolios in a separate array
	// This will allow a deletion of the portfolio
	$scope.picked = [];
	$scope.add_favorite_error_msg = '';

	$scope.portfolios.$loaded().then( function() {

		$scope.maxid = $scope.portfolios.length;

		for (var i = 0; i < $scope.maxid ; i++) {
			$scope.picked[i] = false;
		};

		$scope.getShowStatus = function (id) {
			if ( $scope.picked[id - 1] ) {
				return false;
			} else {
				return true;
			}
		...
		$scope.addFavorite = function (new_favorite) {
			if ( new_favorite.id < 1 || new_favorite.id > $scope.maxid ) {
				$scope.add_favorite_error_msg = 'Invalid id. Please pick an id between 1 and ' + $scope.maxid + '.';
			} else if ( $scope.picked[new_favorite.id - 1] ) {
				$scope.add_favorite_error_msg = '#' + new_favorite.id + ' is already in your favorites. Please pick another photo.';
				$scope.favorite = {};
			} else {
				// To get animation to work, delete portfolio from portfolios, then add to favorites
				var portfolio = $scope.portfolios.splice(new_favorite.id - 1, 1)[0];
				$scope.favorites.push(portfolio);
				$scope.picked[new_favorite.id - 1] = true;
				$scope.favorite = {};
				$scope.add_favorite_error_msg = '';
			};
		};
